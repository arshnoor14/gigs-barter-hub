...
export default function Dashboard() {
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();   // 🔥 line add
  setProfile(data);
      ...
      <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-2xl shadow-xl border border-purple-300 p-8 transform transition-transform hover:-translate-y-1 hover:shadow-2xl">
        ...
        {isEditingProfile ? (
          <form onSubmit={handleProfileSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-purple-800">Skills</label>
              <input
                type="text"
                name="skills"
            ...
            <div>
              <label className="block text-sm font-medium text-purple-800">Languages</label>
              <input
                type="text"
                name="languages"
